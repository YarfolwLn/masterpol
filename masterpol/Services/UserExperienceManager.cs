    ...
    internal class UserExperienceManager
    {
        ...
        public static void CustomizeControls(Control.ControlCollection controls)
        {
            foreach (Control control in controls)
            {
                ...
                {
                    control.BackColor = ColorTranslator.FromHtml(masterpol.Constants.Color.attentionColor);
                }

                if (control.Name.Contains("AttentionFont"))
                {
                    control.ForeColor = ColorTranslator.FromHtml(masterpol.Constants.Color.attentionColor);
                }
            }
        }
